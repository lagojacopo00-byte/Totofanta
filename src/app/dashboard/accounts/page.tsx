 ...
 * docs/06_Database.md, "Identità pubblica facoltativa") — e di
 * eliminare per sempre l'account di un altro utente (non il proprio: per
 * quello resta /play/profile). Niente cambio ruolo o altro da qui. */
    ...
    <div className="flex flex-col gap-6">
      ...
      <ul className="flex flex-col gap-3">
        {profiles.map((p) => {
          ...
          const label = p.displayName || [p.firstName, p.lastName].filter(Boolean).join(" ") || p.email;
          return (
            <li key={p.id} className={`${cardTight} flex flex-col gap-2`}>
              ...
              {p.ownedTournamentNames.length > 0 ? (
                <p className="text-xs text-foreground-faint">
                  Organizza {p.ownedTournamentNames.length} torneo/i (
                  {p.ownedTournamentNames.join(", ")})
                </p>
              ) : null}
              <div className="flex justify-end">
                {isSelf ? (
                  <span className="text-xs text-foreground-faint">
                    Per cancellare il tuo account, vai su Profilo
                  </span>
                ) : (
                  <DeleteAccountButton
                    userId={p.id}
                    label={label}
                    ownedTournamentNames={p.ownedTournamentNames}
                  />
                )}
              </div>
            </li>
